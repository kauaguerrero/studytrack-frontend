import {
  ...
  type SprintSnapshot,
  type TaskRecord,
} from '../src/lib/tasks/intelligence';
...
const tasks: TaskRecord[] = [blockedTask, doneTask];
...
const completeness = calculateTaskCompleteness(blockedTask);
...
const taskSignals = buildTaskSignals(blockedTask, '2026-04-15T12:00:00.000Z');
...
const portfolioSignals = buildPortfolioSignals(tasks, sprints, '2026-04-15T12:00:00.000Z');
assert(portfolioSignals.length > 0, 'Portfolio signals should not be empty');

const metrics = buildDashboardMetrics(tasks, sprints, '2026-04-15T12:00:00.000Z');
