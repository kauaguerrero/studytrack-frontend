...
const StatusBadge = ({ status }: { status: string }) => {
    const configs: any = {
        'completed': { color: 'bg-emerald-100 text-emerald-700 border-emerald-200', icon: CheckCircle2, label: 'Finalizado' },
        'review_required': { color: 'bg-amber-100 text-amber-700 border-amber-200', icon: AlertCircle, label: 'Revisão Necessária' },
        'processing': { color: 'bg-blue-100 text-blue-700 border-blue-200', icon: Loader2, label: 'Processando IA', animate: true },
        'draft': { color: 'bg-slate-100 text-slate-600 border-slate-200', icon: Clock, label: 'Rascunho' },
        'error': { color: 'bg-red-100 text-red-700 border-red-200', icon: AlertCircle, label: 'Falha no Processo' }
    };
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1 font-medium">Gestão centralizada de inclusão e acessibilidade.</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Link href="/portal/secretariat/adaptation/new">
              <Button className="bg-slate-900 hover:bg-blue-600 text-white shadow-lg shadow-slate-900/20 transition-all active:scale-95 h-10 px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card className="border-slate-200 shadow-[0_2px_10px_rgba(0,0,0,0.04)] hover:shadow-md transition-shadow">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-bold text-slate-500 uppercase tracking-wider">Solicitações Hoje</CardTitle>
                    ...
                    <Card className="border-slate-200 shadow-[0_2px_10px_rgba(0,0,0,0.04)] hover:shadow-md transition-shadow">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-bold text-slate-500 uppercase tracking-wider">Atenção Necessária</CardTitle>
                            <AlertCircle className="h-4 w-4 text-amber-500" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-3xl font-black text-amber-600">{stats.pending}</div>
                            <p className="text-xs text-slate-500 mt-1">Aguardando revisão técnica</p>
                        </CardContent>
                    </Card>
                    <Card className="border-slate-200 shadow-[0_2px_10px_rgba(0,0,0,0.04)] hover:shadow-md transition-shadow">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-bold text-slate-500 uppercase tracking-wider">Total Processado</CardTitle>
                            <CheckCircle2 className="h-4 w-4 text-blue-500" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-3xl font-black text-blue-600">{stats.total}</div>
                            <p className="text-xs text-slate-500 mt-1">Documentos adaptados este mês</p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    {jobs.length === 0 ? (
                        <div className="text-center py-20">
                            ...
                            <h4 className="text-slate-900 font-bold">Nenhum registro encontrado</h4>
                            <p className="text-slate-500 text-sm mt-1">Comece criando uma nova solicitação de adaptação.</p>
                        </div>
                    ) : (
                                    ...
                                    <div className="flex items-center gap-6">
                                        ...
                                        <Link href={`/portal/secretariat/adaptation/${job.id}`}>
                                            <Button variant="ghost" size="sm" className="text-slate-500 hover:text-blue-600 hover:bg-blue-50">
                                                Gerenciar <ArrowUpRight size={14} className="ml-1" />
                                            </Button>
                                        </Link>
                                    </div>
                                </div>
