...
export async function POST(request: Request) {
  ...
  if (requestContentType.includes('multipart/form-data')) {
    ...
    alternativeLetter = String(formData.get('alternativeLetter') || '').trim().toUpperCase();
    ...
    alternativeLetter = String(payload.alternativeLetter || '').trim().toUpperCase();
  ...
  if (!questionId || !alternativeLetter) {
    return NextResponse.json(
      { error: 'questionId e alternativeLetter são obrigatórios.' },
      { status: 400 }
    );
  }
