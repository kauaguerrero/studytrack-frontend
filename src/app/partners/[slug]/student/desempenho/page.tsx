...
function buildActivityChart(
  ...
  const questionMap = new Map(
    history.map((item) => [
      item.usage_date,
      {
        ...
        simulations: item.simulations_count,
      },
    ]),
  );

  const essayMap = new Map<string, number>();
  correctedEssays.forEach((essay) => {
    const key = (essay.corrected_at || essay.submitted_at || '').slice(0, 10);
    if (!key || essay.total_score == null) return;
    essayMap.set(key, essay.total_score);
  });

  const allKeys = Array.from(new Set([...questionMap.keys(), ...essayMap.keys()])).sort();
  ...
  return allKeys.slice(-12).map((key) => {
    ...
    return {
      ...
      essayScore: essayMap.get(key) ?? null,
    };
  });
}
...
function buildHabits(history: AnalyticsResponse['activity_history']) {
  ...
  if (delta >= 12) {
    rhythmLabel = 'Acelerando';
    rhythmTone = 'up';
  } else if (delta <= -12) {
    rhythmLabel = 'Em queda';
    rhythmTone = 'down';
  }
...
export default function DesempenhoPage() {
  ...
  const [visibleEvolutionMetrics, setVisibleEvolutionMetrics] = useState<Array<'questions' | 'simulations'>>([
    'questions',
    'simulations',
  ]);
    ...
    async function load() {
      ...
      try {
        const [analyticsRes, essaysRes, summaryRes, rankingRes, simuladoRes] = await Promise.all([
          fetch(`${apiUrl}/api/student/analytics/dashboard`, { headers, cache: 'no-store' }),
          fetch(`${apiUrl}/api/partners/${slug}/essays?status=all&page=1&limit=60`, { headers, cache: 'no-store' }),
          fetch(`${apiUrl}/api/partner/gamification/summary`, { headers, cache: 'no-store' }),
          fetch(`${apiUrl}/api/partner/gamification/ranking?limit=10`, { headers, cache: 'no-store' }),
          fetch(`${apiUrl}/api/simulado/history?page=1&limit=12`, { headers, cache: 'no-store' }),
        ]);
        ...
        const detailResponses = await Promise.all(
          correctedRecent.map(async (essay) => {
            const response = await fetch(`${apiUrl}/api/partners/${slug}/essays/${essay.id}`, { headers, cache: 'no-store' });
            if (!response.ok) return null;
            const payload: EssayDetail = await response.json();
            return payload;
          }),
        );
        ...
        setState({
          analytics,
          essays,
          essayDetails: detailResponses.filter((item): item is EssayDetail => Boolean(item))
            .sort((a, b) => new Date(a.submitted_at).getTime() - new Date(b.submitted_at).getTime()),
          summary,
          ranking,
  ...
  const derived = useMemo(() => {
    ...
    const latestEssayDetail = latestEssay
      ? essayDetails.find((item) => item.id === latestEssay.id) || null
      : null;
    ...
    const activityChart = buildActivityChart(analytics.activity_history, correctedEssays);
  ...
  const subjectRows = analytics.performance_by_subject
    .slice()
  ...
  function toggleEvolutionMetric(metric: 'questions' | 'simulations') {
          ...
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-12">
            <KPI
              ...
              hint={summary?.rank_position
                ? `${summary.rank_position <= (summary.prize_cutoff || 3) ? 0 : (summary.points_to_top3 || 0)} pts do top ${summary.prize_cutoff || 3}`
                : 'Sem posição'}
              icon={Trophy}
              accent="#f59e0b"
            />
              ...
              hint={latestEssay
                ? (
                  <span className="block">
                    Última {latestEssay.total_score} • melhor {bestEssay?.total_score ?? '—'}
                  </span>
                )
                : 'Sem correção'}
              icon={FilePenLine}
                ...
                <div className={cn(SOFT_CARD_CLASS, 'w-full px-4 py-3 text-sm text-slate-600 sm:w-auto dark:text-slate-200')}>
                  {essayTrendDelta != null
                    ...
                    : correctedEssays.length > 0
                      ? 'Primeiras correções em andamento'
              ...
              <div className="mt-6 h-[260px] w-full sm:h-[320px]">
                {filteredActivityChart.length ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={filteredActivityChart} margin={{ left: -24, right: 4, top: 12, bottom: 0 }}>
                      <defs>
                      ...
                      <Tooltip
                        content={({ active, payload }) => {
                          ...
                          const row = payload[0]?.payload as { fullDate: string; questions: number; simulations: number };
                          return (
              ...
              <div className="mt-6 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
                <div className={cn(CHART_CARD_CLASS, 'h-[250px] sm:h-[280px]')}>
                  {correctedEssays.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={correctedEssays.map((essay) => ({
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            <p className="text-sm font-semibold text-slate-900 dark:text-white">
                              C{item.competency} · {item.label}
                            </p>
                            <p className="mt-1 text-xs text-slate-500 dark:text-slate-300">
                              {item.count ? `média de ${item.count}` : 'sem nota ainda'}
                            </p>
                          </div>
                          ...
                          <div
                            ...
                            style={{
                              ...
                              background: item.count
                                ? item.competency === weakestCompetency?.competency
                                  ? '#e11d48'
                                  : item.competency === strongestCompetency?.competency
                                    ? '#0f766e'
                                    : org.brand_primary
                                : '#94a3b8',
                            }}
              ...
              <div className="mt-5 grid gap-3 lg:grid-cols-2">
                <div className={SOFT_CARD_CLASS}>
                  ...
                  <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                    {weakestCompetency
                      ? `${weakestCompetency.score} / 200 na redação mais recente`
                      : 'Assim que houver uma correção, este bloco aparece.'}
                  </p>
                </div>
                ...
                <div className={SOFT_CARD_CLASS}>
                  ...
                  <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                    {strongestCompetency
                      ? `${strongestCompetency.score} / 200 na redação mais recente`
                      : 'Assim que houver uma correção, este bloco aparece.'}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <div className={SOFT_CARD_CLASS}>
                  ...
                  <p className={cn(
                    ...
                    habits.rhythmTone === 'up'
                      ? 'text-emerald-600 dark:text-emerald-300'
                      : habits.rhythmTone === 'down'
