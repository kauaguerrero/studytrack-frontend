...
// ── Confetti: mesmas partículas do QuestionSessionRewardPopup/SimuladoRewardPopup ──
...
const confettiKeyframes = PARTICLES.map((p) => {
  ...
  return `@keyframes rbr-confetti-${p.id} {
...
export function ReportBountyRewardPopup({ points, reportCount, slug, onDismiss }: Props) {
  ...
  const goToReports = () => {
    onDismiss();
    router.push(`/partners/${slug}/student/suporte/reports`);
  };
  ...
  return (
    <div
      ...
      aria-live="polite"
      aria-label="Pontos ganhos por report de bug confirmado"
    >
          ...
          {PARTICLES.map((p) => (
            <div
              ...
              style={{
                ...
                animation: `rbr-confetti-${p.id} ${p.durationS}s ease-out both`,
              }}
            />
          ))}
        </div>
        ...
        <div className="overflow-y-auto px-5 pt-7 pb-5 space-y-5 sm:px-6 sm:pt-8 sm:pb-6">
          ...
          <div className="text-center">
            ...
            <p className={`mt-1 text-sm ${theme.bodyClass}`}>
              {isMultiple
                ? `${reportCount} reports seus ajudaram a corrigir questões`
                : 'Seu report ajudou a corrigir uma questão'}
            </p>
          </div>
          ...
          <div className="rounded-2xl px-4 py-3" style={theme.accentPanelStyle}>
            ...
            <p className={`mt-1 text-sm ${theme.bodyClass}`}>
              Um admin revisou seu report e confirmou o erro. Pontos já somados ao seu ranking mensal.
            </p>
          </div>
          ...
          >
            {isMultiple ? 'Ver devolutivas →' : 'Ver devolutiva →'}
          </button>
        </div>
      </motion.div>
    </div>
