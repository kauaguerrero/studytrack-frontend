...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ generated_at: payload.generated_at, signals: payload.signals });
  } catch (error: any) {
    return NextResponse.json({ error: error?.message || 'Falha ao gerar sinais' }, { status: 500 });
  }
}
