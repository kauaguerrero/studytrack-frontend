...
export function DashboardNavbar({ firstName, fullName, userRole, avatarUrl }: DashboardNavbarProps) {
  ...
  return (
    <>
      {/* --- NAVBAR FIXA --- */}
      <nav className={`sticky top-0 z-40 px-4 pt-4 pb-2 transition-all duration-300 ${isMobileMenuOpen ? 'z-50' : ''}`}>
        <div className={`
        ...
        `}>
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <form action="/auth/signout" method="post">
              <button 
                className="p-2.5 hover:bg-red-50 hover:text-red-500 rounded-xl text-slate-400 transition-all active:scale-95 border border-transparent hover:border-red-100"
      ...
      <div 
        className={`
          fixed inset-y-0 left-0 z-50 w-[280px] bg-white shadow-2xl transform transition-transform duration-300 ease-out md:hidden flex flex-col
